...
JSX is a syntax that lets us write HTML-like code inside JavaScript or TypeScript. It makes React components easier to read and helps us describe what the user interface should look like.
...
`useEffect` is used for side effects such as fetching data, working with timers, or communicating with something outside the component after rendering.

In this project, I did not use `useEffect` to load the local JSON file. I created a Promise for the fetch request and used React's `use()` with `Suspense` to read the Promise and show a loading fallback while the data was loading.
